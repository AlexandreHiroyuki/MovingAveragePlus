...
private:
  ...
  TypeOfArray _update_partial_sum()
  {
    for (size_t i = 0; i < _partial_sums_counter; i++)
    {
      _partial_sums[i] += (*this)[0];
...
public:
  ...
  MovingAverage<TypeOfArray> &push(TypeOfArray input)
  {
    ...
    for (size_t i = 0; i < _partial_sums_counter; i++)
    {
      ...
      _partial_sums[i] -= (_current_index == _partial_sum_sizes[i] - 1) ? last_value : (*this)[_partial_sum_sizes[i] - 1];
    }
  ...
  {
    if (n_points > _average_counter)
      n_points = _average_counter;
  ...
  size_t create_partial(size_t sum_size)
  {
    ...
    {
      _partial_sums = (TypeOfArray *)realloc(_partial_sums, _partial_sums_counter * sizeof(TypeOfArray));
      _partial_sum_sizes = (TypeOfArray *)realloc(_partial_sum_sizes, _partial_sums_counter * sizeof(TypeOfArray));
    }
  ...
  TypeOfArray get_partial(size_t id)
  {
    if (_average_counter >= _partial_sum_sizes[id])
      ...
      return _partial_sums[id] / ((_average_counter == 0) ? 1 : _average_counter);
  }
};
